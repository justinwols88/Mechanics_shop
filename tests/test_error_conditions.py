...
class ErrorConditionsTestCase(unittest.TestCase):
    ...
    def setup_test_data(self):
        ...
        self.inventory1 = Inventory(part_name="Brake Pads", price=49.99)  # Changed from 'name'
        self.inventory2 = Inventory(part_name="Oil Filter", price=12.99)  # Changed from 'name'
        ...
        self.ticket2 = ServiceTicket(
            description="Oil change requested", 
            customer_id=1, 
            status="in_progress"
        )
        ...
        db.session.add_all([
            ...
            self.mechanic1, self.mechanic2,
            self.inventory1, self.inventory2,
            self.ticket1, self.ticket2
        ])
        db.session.commit()
    ...
    def test_400_missing_required_fields_service_ticket(self):
        ...
        response = self.client.post('/tickets', 
                                  ...
                                  json={'description': 'Test ticket'})  # Missing customer_id
        self.assertEqual(response.status_code, 400)
    ...
    def test_400_invalid_data_types_inventory(self):
        ...
        response = self.client.post('/inventory/', 
                                  json={'part_name': 'Test Part', 'price': 'invalid'})  # Use   'part_name' not 'name'
        self.assertEqual(response.status_code, 400)
        data = json.loads(response.data)
        self.assertIn('Price must be a valid number', data['errors'])  # Updated error message
    
        # Price as negative number
        response = self.client.post('/inventory/', 
                                  json={'part_name': 'Test Part', 'price': -10.00})  # Use  'part_name' not 'name'
        self.assertEqual(response.status_code, 400)
        data = json.loads(response.data)
        ...
        response = self.client.post('/inventory/', 
                                  json={'part_name': '', 'price': 10.00})  # Use 'part_name' not    'name'
        self.assertEqual(response.status_code, 400)
        data = json.loads(response.data)
        self.assertIn('part_name is required and cannot be empty', data['errors'])  # Updated error message
    ...
    def test_401_wrong_token_type_for_endpoint(self):
        ...
        response = self.client.delete(f'/tickets/{self.ticket1.id}', 
                                headers={'Authorization': f'Bearer {self.customer_token}'})
    ...
    def test_404_nonexistent_customer(self):
        ...
        response = self.client.put('/customers/9999', json={'email': 'updated@example.com'})
        self.assertEqual(response.status_code, 404)
    ...
    def test_404_nonexistent_service_ticket(self):
        ...
        response = self.client.delete('/tickets/9999', 
                                    headers={'Authorization': f'Bearer {self.mechanic_token}'})
        self.assertEqual(response.status_code, 404)
        
        response = self.client.put('/tickets/9999/edit', 
                                 json={'add_ids': [1]})
        self.assertEqual(response.status_code, 404)
        
        response = self.client.post('/tickets/9999/add-part', 
                                  json={'part_id': 1})
        self.assertEqual(response.status_code, 404)
...
def test_404_nonexistent_inventory(self):
    ...
    response = self.client.patch('/inventory/999/archive', 
                               headers={'Authorization': f'Bearer {token}'})
    self.assertEqual(response.status_code, 404, "PATCH /inventory/999/archive should return 404")

    def test_404_nonexistent_part_in_ticket(self):
        """Test 404 when adding nonexistent part to ticket"""
        response = self.client.post(f'/tickets/{self.ticket1.id}/add-part', 
                                  json={'part_id': 9999})
        self.assertEqual(response.status_code, 404)
    ...
    def test_409_duplicate_inventory_name(self):
        ...
        response = self.client.post('/inventory/', 
                                  json={'name': 'Brake Pads', 'price': 59.99})
        self.assertEqual(response.status_code, 409)
        data = json.loads(response.data)
    ...
    def test_business_validation_errors_inventory(self):
        ...
        # Very high price
        response = self.client.post('/inventory/', 
                                  json={'name': 'Test Part', 'price': 9999999.99})
        self.assertEqual(response.status_code, 400)
        data = json.loads(response.data)
    ...
    def test_invalid_pagination_parameters(self):
        ...
        response = self.client.get('/customers/all?page=0&per_page=5')  # page should be >= 1
        # This might return 400 or default to page 1
        
        response = self.client.get('/customers/all?page=-1&per_page=5')
        # This might return 400 or default to page 1
        
        response = self.client.get('/customers/all?page=1&per_page=0')  # per_page should be >= 1
    ...
    def test_large_input_values(self):
        ...
        response = self.client.post('/inventory/', 
                                  json={'name': 'Test', 'price': 999999.99})
    ...
    def test_all_endpoints_have_error_handling(self):
        ...
        for endpoint, method, expected_status in endpoints_to_test:
            ...
            self.assertIsNotNone(
                response,
                f"Failed to create response object for {method} {endpoint}"
            )
