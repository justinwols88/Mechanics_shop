...
class MechanicsTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.ticket1 = ServiceTicket(description="Ticket 1", customer_id=self.customer.id)
        self.ticket2 = ServiceTicket(description="Ticket 2", customer_id=self.customer.id)
    ...
    def test_mechanics_ranking(self):
        ...
        if len(mechanics_data) >= 2:
            self.assertGreaterEqual(mechanics_data[0]['ticket_count'], mechanics_data[1]['ticket_count'])
