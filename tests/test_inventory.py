...
class InventoryTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.inventory_item = Inventory(part_name="Brake Pads", price=49.99)  # Changed 'name' to 'part_name' and 'price' to 'cost'
    ...
    def test_create_inventory_success(self):
        ...
        response = self.client.post('/inventory/',
                                  json={'part_name': 'Oil Filter', 'price': 12.99})
    ...
    def test_create_inventory_invalid_price(self):
        ...
        response = self.client.post(
            '/inventory/',
            json={'part_name': 'Test Part', 'price': -10.00}
        )
        self.assertEqual(response.status_code, 400)
    ...
    def test_archive_inventory_authenticated(self):
        ...
        response = self.client.patch(f'/inventory/{self.inventory_item.id}/archive', 
                                   headers={'Authorization': f'Bearer {token}'})
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.data)
        self.assertTrue(data['success'])
